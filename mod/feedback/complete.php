...
//print_object($feedbackcompletion);
   ...
   // print_object($feedback);
    if ($feedbackcompletion->just_completed()) {
       // print_object($feedback);
        ...
        //Change by Sangita........................here i have checked feedback name 
        //print_object($feedback->name);
        if($feedback->name == 'Heart Patient'){

               $feeddata = $feedbackcompletion->get_completed();
              // print_object($feeddata);
               $compid = $feeddata->id;
               $fid = $feeddata->feedback;
               $fvalue = $DB->get_record('feedback',array('id' =>$fid));
              // print_object($compid);
               $patientdetails = $DB->get_record('patient_complete_details',array('completed_id' =>$compid));
               $patientscoredetails = $DB->get_record('patient_score_details',array('completed_id' =>$compid));
               $sdq =  $patientscoredetails ->sdq;
               $imp01 =  $patientscoredetails ->impact01;
               $imp02 =  $patientscoredetails ->impact02;
               //print_object($patientdetails);
               $patientid = $patientdetails->patient_id;
              // print_object($patientid);
               $patint = 'New Patient Details :';
               if($fvalue->name == 'Heart Patient'){

                   echo $OUTPUT->container_start();
                   echo $patint.' '.$patientid ."<br>";
                   echo "Patient SDQ Total Score :" .' '. $sdq ."<br>";
                   echo "Patient Impact01 Total Score :" .' '. $imp01 ."<br>";
                   echo "Patient Impact02 Total Score :" .' '. $imp02 ."<br><br><br>";
                   echo $OUTPUT->container_end();
                   echo $OUTPUT->container_start();
                   echo "<button onclick= 'return fetchvalue(1,2,3);' id='myButton' class='float-left submit-button' >Download Report</button>";
              ?>
                  <script type="text/javascript">
                      function abc(val){
                       //alert(val);
                      window.location = "scoretable.php?id=<?php echo $compid ?>&courseid=<?php echo $course->id?>&type="+val;
                    }
                  function fetchvalue(val1,val2,val3){
                      window.location = "scoretable.php?id=<?php echo $compid ?>&courseid=<?php echo $course->id?>&typeval="+val1+','+val2+','+val3;
                       }
                  ...
                  echo $OUTPUT->container_end();
                   ...
                   echo $OUTPUT->container_start();
                   echo "Enter Heart Patient Details Only";
                   echo $OUTPUT->container_end();
                 }
     ...
     //...........................................
       //echo $OUTPUT->heading(format_string($patientid));
