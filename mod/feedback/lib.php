...
//Sangita(Function Created by me)

function score_calculation($fcompid ,$categoryvalue){
  global $DB;
  ...
  if($i == 0 OR $i == 6 OR $i == 12){
     ...
     $table->head = array('Subscale','Not True','Somewhat true','Certainly true');
  }
     
 // $table->align = array('left','left','left','left');
  // $table->align = array('','','','');
  //$table->size = array('20%');
  //$table->cellpadding = array('20%');
  if($i == 0 OR $i==6 OR $i == 12)
  {
     $heading = 'Emotional problems subscale '; 
   
  }elseif ($i == 1 OR $i == 7 OR $i == 13) {

     $heading = 'Conduct problems subscale ';
    
  }elseif ($i == 2 OR $i == 8 OR $i == 14) {

     $heading = 'Hyperactivity subscale';
    
  }elseif($i == 3 OR $i == 9 OR $i == 15){

     $heading = 'Peer problems subscale';
   
  } elseif ($i == 4 OR $i == 10 OR $i == 16) {
    
      $heading = 'Prosocial subscale';
   
  } 
  
  $totalvalue = 'Total';
  
  $firstsubheading = new html_table_cell();
  $firstsubheading->text = $heading;////lang file
  $firstsubheading->scope = 'row';
  $firstsubheading->header = true;
  $firstsubheading->colspan = 4;
  $hedingrow = new html_table_row();
  $hedingrow->cells = array($firstsubheading);
  $table->data[] = $hedingrow;

  $sumval2 = 0;
  foreach ($category as $value1) {
       
    $lablerecord = $DB->get_record('feedback_item',array('label' =>$value1));
    $lid =  $lablerecord->id;
    $lablevalue = $DB->get_record('feedback_value',array('item' =>  $lid , 'completed' => $fcompid));
    $ansvalue = $lablevalue->value;
    $equalans = $ansvalue + 1;
    $stringvalue = explode("-", $value1);
    unset($stringvalue[0]);
    $svalue1 = $stringvalue[1];
    $svalue2 = $stringvalue[2];
    $svalue3 = $stringvalue[3];
    $svalue4 = $stringvalue[4];
   // $svalue5 = 1;
    ...
    if($stringvalue[2] == $stringvalue[$equalans]){

       $str1 = "<i class='fa fa-check'></i>";
       $sumval1 = $sumval1 + $svalue2;
       $tablevalue = array($svalue1,$svalue2.$str1,$svalue3,$svalue4);

    }elseif($stringvalue[3] == $stringvalue[$equalans]){
       
        $str2 = '<i class="fa fa-check"></i>';
        $sumval2 = $sumval2 + $svalue3;
        $tablevalue = array($svalue1,$svalue2,$svalue3.$str2,$svalue4);

    }elseif($stringvalue[4] == $stringvalue[$equalans]){

       $str3 = '<i class="fa fa-check"></i>';
       $sumval3 = $sumval3 +$svalue4;
       $tablevalue = array($svalue1,$svalue2,$svalue3,$svalue4.$str3);

    }else{

       $tablevalue = array($svalue1,$svalue2,$svalue3,$svalue4); 
    } 
  ...
  $calculationheading = new html_table_cell();
  $calculationheading->text = 'Total';////lang file
  $cal1 = new html_table_cell();
  $cal1->text = $sumval1;
  $cal2 = new html_table_cell();
  $cal2->text = $sumval2;
  $cal3 = new html_table_cell();
  $cal3->text = $sumval3;
  $calculationheading->header = true;
  $calhedingrow = new html_table_row();
  $calhedingrow->cells = array($calculationheading,$cal1,$cal2,$cal3);
  $table->data[] = $calhedingrow;

  $tablebody .= html_writer::table($table);

  $sub = get_string('Subscale', 'feedback');
  $scoring = get_string('scoring', 'feedback'); 
  $int = get_string('Inter', 'feedback'); 
  $ext = get_string('Exter', 'feedback'); 
  $total = get_string('Total', 'feedback'); 
  $sum1 = get_string('Sum1', 'feedback'); 
  $sum2 = get_string('Sum2', 'feedback'); 
  $sum3 = get_string('Sum3', 'feedback');
  $score = get_string('Score', 'feedback'); 

  static $totalscore01 = 0;
  static $totalscore02 = 0;
  static $totalscore03 = 0;
  static $totalscore04 = 0;
  static $totalscore05 = 0;
  static $totalscore06 = 0;
  static $totalscore07 = 0;
  static $totalscore08 = 0;
  static $totalscore09 = 0;

//first calculation table..................................
  if($i == 0 OR $i == 3){

   $totalscore01 = $totalscore01 + $sumval1 + $sumval2 + $sumval3;
   //print_object($totalscore01);
  }
 
 if($i == 1 OR $i == 2)
 {
    $totalscore02 = $totalscore02 + $sumval1 + $sumval2 + $sumval3;
     //print_object($totalscore02);
 }

 if($i == 4)
 {

    $totalscore03 = $totalscore03 + $sumval1 + $sumval2 + $sumval3;
    //print_object($totalscore03);
    
     
 } 
//Second calculation table........................
 if($i == 6 OR $i == 9){
    $totalscore04 = $totalscore04 + $sumval1 + $sumval2 + $sumval3;
     //print_object($totalscore04);

 }
 if($i == 7 OR $i == 8 ){
    $totalscore05 = $totalscore05 + $sumval1 + $sumval2 + $sumval3;
     //print_object($totalscore05);

 }  
 if($i == 10){

     $totalscore06 = $totalscore06 + $sumval1 + $sumval2 + $sumval3;
      //print_object($totalscore06);

 }
//Third calculation table........................
 if($i == 12 OR $i == 15){
    $totalscore07 = $totalscore07 + $sumval1 + $sumval2 + $sumval3;
     //print_object($totalscore07);

 }
 if($i == 13 OR $i == 14 ){
    $totalscore08 = $totalscore08 + $sumval1 + $sumval2 + $sumval3;
    // print_object($totalscore08);

 }  
 if($i == 16){

     $totalscore09 = $totalscore09 + $sumval1 + $sumval2 + $sumval3;
     // print_object($totalscore09);
 ...
 // $int = 
  ...
  $table = new html_table();
  $table->width = '100%';
  $table->head = array( $sub,$int,$ext,$total);
  $table->size = array('20%');
  $cel1 = new html_table_cell();
  $cel1->text = $scoring;
  $cel2 = new html_table_cell();
  $cel2->text = $sum1;
  $cel3 = new html_table_cell();
  $cel3->text = $sum2;
  $cel4 = new html_table_cell();
  $cel4->text = $sum3;
  $celrow = new html_table_row();
  $celrow->cells = array($cel1,$cel2,$cel3,$cel4);
  $table->data[] = $celrow;

  $cel5 = new html_table_cell();
  $cel5->text = $score;
  $cel6 = new html_table_cell();
  //$cel6->text = $totalscore01;
 // print_object($cel6->text);
  $cel7 = new html_table_cell();
  //$cel7->text = $totalscore02;
  $cel8 = new html_table_cell();
  // $cel8->text = $totalscore03;
  if($i == 5){
  //  print_object($cel6->text);
    $cel6->text = $totalscore01;
    $cel7->text = $totalscore02;
  ...
  if($i == 11){
   // print_object("second Score");
    $cel6->text = $totalscore04;
    $cel7->text = $totalscore05;
    $cel8->text = $totalscore06;
  }
  if($i == 17 ){
   // print_object("third Score");
     $cel6->text = $totalscore07;
    $cel7->text = $totalscore08;
    $cel8->text = $totalscore09;
  }
  $celrow1 = new html_table_row();
  $celrow1->cells = array($cel5,$cel6,$cel7,$cel8);
  $table->data[] = $celrow1;

  $tablebody1 .= html_writer::table($table);
 ...
 // print_object($i);
  ...
  // return ['table1' => $tablebody, 'table2' => $tablebody1]; 
  return($tablebody1);
