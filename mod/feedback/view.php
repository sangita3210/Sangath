...
$stdid = optional_param('stdid',false,PARAM_RAW);
global $USER,$DB;
//print_object($USER);
...
if ($feedbackcompletion->can_complete()) {
    ...
    if (!$feedbackcompletion->is_open()) {
        ...
        ///Changed by Sangita
       // print_object($feedback->name);
        if($feedback->name == 'Second session'){

        	 if($stdid == null){

	    		//$pages = ['abc','xyz','pqr'];
	    		$Patientrecords = $DB->get_records('patient_complete_details',array('consoler_username'=> $username));
	    		//print_object($pages);
		        $selectoptions = array();
		        ...
		        foreach ($Patientrecords as $Patientrecord) {
		        	$pid = $Patientrecord->patient_id;
		        	$selectoptions[$pid] = $pid;
		        }
		        $label = 'Select Patient Id'. ': ';
		        $select = new single_select(new moodle_url('/mod/feedback/view.php?id='.$id), 'stdid', $selectoptions);
		        $select->label = $label;
