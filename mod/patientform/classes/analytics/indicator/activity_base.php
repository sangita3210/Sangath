...
abstract class activity_base extends \core_analytics\local\indicator\community_of_inquiry_activity {
     ...
     * @return bool
     */
    protected function patientform_viewed(\cm_info $cm, $contextid, $userid, $after = null) {
        // If stats are published any write action counts as viewed patientform.
        if (!empty($this->instancedata[$cm->instance]->publish_stats)) {
            $user = (object)['id' => $userid];
            return $this->any_write_log($contextid, $user);
        }

        return parent::patientform_viewed($cm, $contextid, $userid, $after);
    }
