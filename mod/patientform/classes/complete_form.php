...
/**
 * Contains class mod_patientform_complete_form
 *
...
/**
 * Class mod_patientform_complete_form
 *
 * @package   mod_patientform
 * @copyright 2016 Marina Glancy
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_patientform_complete_form extends moodleform {
    ...
    /** @var int */
    protected $mode;
    /** @var mod_patientform_structure|mod_patientform_completion */
    protected $structure;
    /** @var mod_patientform_completion */
     ...
     *
     * @param int $mode
     * @param mod_patientform_structure $structure
     * @param string $formid CSS id attribute of the form
     * @param array $customdata
     */
    public function __construct($mode, mod_patientform_structure $structure, $formid, $customdata = null) {
        $this->mode = $mode;
        ...
        parent::__construct(null, $customdata, 'POST', '',
                array('id' => $formid, 'class' => 'patientform_form' . $isanonymous), true);
    ...
    public function definition() {
        ...
        if (isloggedin() && !isguestuser() && $this->mode != self::MODE_EDIT && $this->mode != self::MODE_VIEW_TEMPLATE &&
            ...
            if ($this->structure->is_anonymous()) {
                $anonymousmodeinfo = get_string('anonymous', 'patientform');
            } else {
                $anonymousmodeinfo = get_string('non_anonymous', 'patientform');
            }
            $element = $mform->addElement('static', 'anonymousmode', '',
                    get_string('mode', 'patientform') . ': ' . $anonymousmodeinfo);
            $element->setAttributes($element->getAttributes() + ['class' => 'patientform_mode']);
        }

        // Add buttons to go to previous/next pages and submit the patientform.
        if ($this->mode == self::MODE_COMPLETE) {
            $buttonarray = array();
            $buttonarray[] = &$mform->createElement('submit', 'gopreviouspage', get_string('previous_page', 'patientform'));
            $buttonarray[] = &$mform->createElement('submit', 'gonextpage', get_string('next_page', 'patientform'),
                    array('class' => 'form-submit'));
            $buttonarray[] = &$mform->createElement('submit', 'savevalues', get_string('save_entries', 'patientform'),
                    array('class' => 'form-submit'));
     ...
     *
     * This will add only items from a current page to the patientform and adjust the buttons
     */
    protected function definition_complete() {
        if (!$this->structure instanceof mod_patientform_completion) {
    ...
    protected function definition_preview() {
        foreach ($this->structure->get_items() as $patientformitem) {
            $itemobj = patientform_get_item_class($patientformitem->typ);
            $itemobj->complete_form_element($patientformitem, $this);
        }
    }
    ...
    /**
     * Can be used by the items to get the course id for which patientform is taken
     *
     * This function returns 0 for patientform that are located inside the courses.
     * $this->get_patientform()->course will return the course where patientform is located.
     * $this->get_current_course_id() will return the course where user was before taking the patientform
     *
     * @return int
     */
    ...
    /**
     * Record from 'patientform' table corresponding to the current patientform
     * @return stdClass
     */
    public function get_patientform() {
        return $this->structure->get_patientform();
    }

    /**
     * Current patientform mode, see constants on the top of this class
     * @return int
     */
    ...
    /**
     * Returns the course where user was before taking the patientform.
     *
     * For patientforms inside the course it will be the same as $this->get_patientform()->course.
     * For patientforms on the frontpage it will be the same as $this->get_course_id()
     *
     * @return int
     */
    public function get_current_course_id() {
        return $this->structure->get_courseid() ?: $this->get_patientform()->course;
    }
    ...
    protected function get_suggested_class($item) {
        $class = "patientform_itemlist patientform-item-{$item->typ}";
        if ($item->dependitem) {
            $class .= " patientform_is_dependent";
        }
        if ($item->typ !== 'pagebreak') {
            $itemobj = patientform_get_item_class($item->typ);
            if ($itemobj->get_hasvalue()) {
                $class .= " patientform_hasvalue";
            }
        }
        return $class;
    }
    ...
    /**
     * Adds an item number to the question name (if patientform autonumbering is on)
     * @param stdClass $item
     * @param HTML_QuickForm_element $element
     */
    protected function add_item_number($item, $element) {
        if ($this->get_patientform()->autonumbering && !empty($item->itemnr)) {
    ...
    protected function guess_element_id($item, $element) {
        if (!$id = $element->getAttribute('id')) {
            $attributes = $element->getAttributes();
            $id = $attributes['id'] = 'patientform_item_' . $item->id;
            $element->setAttributes($attributes);
        }
    ...
    protected function enhance_name_for_edit($item, $element) {
        ...
        $menu->set_constraint('.patientform_form');
        $menu->set_alignment(action_menu::TR, action_menu::BR);
        $menu->set_menu_trigger(get_string('edit'));
        $menu->prioritise = true;

        $itemobj = patientform_get_item_class($item->typ);
        $actions = $itemobj->edit_actions($item, $this->get_patientform(), $this->get_cm());
        ...
        $name = html_writer::span('', 'itemdd', array('id' => 'patientform_item_box_' . $item->id)) .
    ...
    public function display() {
        ...
        if ($this->mode == self::MODE_EDIT) {
            $PAGE->requires->js_call_amd('mod_patientform/edit', 'setup');
        }
    }
}
