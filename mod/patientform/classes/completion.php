...
/**
 * Contains class mod_patientform_completion
 *
...
/**
 * Collects information and methods about patientform completion (either complete.php or show_entries.php)
 *
 * @package   mod_patientform
 * @copyright 2016 Marina Glancy
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_patientform_completion extends mod_patientform_structure {
    /** @var stdClass */
    protected $completed;
    /** @var stdClass */
    ...
    /** @var mod_patientform_complete_form the form used for completing the patientform */
    protected $form = null;
    /** @var bool true when the patientform has been completed during the request */
     ...
     * @param cm_info $cm course module object corresponding to the $patientform
     *     (at least one of $patientform or $cm is required)
     * @param int $courseid current course (for site patientforms only)
     * @param bool $iscompleted has patientform been already completed? If yes either completedid or userid must be specified.
     * @param int $completedid id in the table patientform_completed, may be omitted if userid is specified
     *     but it is highly recommended because the same user may have multiple responses to the same patientform
     ...
     */
    public function __construct($patientform, $cm, $courseid, $iscompleted = false, $completedid = null, $userid = null) {
        global $DB;
        parent::__construct($patientform, $cm, $courseid, 0);
        // Make sure courseid is always set for site patientform.
        if ($this->patientform->course == SITEID && !$this->courseid) {
        ...
        if ($iscompleted) {
            ...
            if (!$userid && !$completedid) {
                throw new coding_exception('Either $completedid or $userid must be specified for completed patientforms');
            }
            if ($completedid) {
                $params['id'] = $completedid;
            }
            if ($userid) {
                // We must respect the anonymousity of the reply that the user saw when they were completing the patientform,
                // not the current state that may have been changed later by the teacher.
                $params['anonymous_response'] = PATIENTFORM_ANONYMOUS_NO;
                $params['userid'] = $userid;
            }
            $this->completed = $DB->get_record('patientform_completed', $params, '*', MUST_EXIST);
            $this->courseid = $this->completed->courseid;
        }
    }

    /**
     * Returns a record from 'patientform_completed' table
     * @return stdClass
     */
    ...
    /**
     * Check if the patientform was just completed.
     *
     * @return bool true if the patientform was just completed.
     * @since  Moodle 3.3
     */
     ...
     *
     * @return stdClass|false record from patientform_completedtmp or false if not found
     */
    public function get_current_completed_tmp() {
        global $USER, $DB;
        if ($this->completedtmp === null) {
            $params = array('patientform' => $this->get_patientform()->id);
            ...
            $this->completedtmp = $DB->get_record('patientform_completedtmp', $params);
        }
    ...
    protected function can_see_item($item) {
        ...
        $itemobj = patientform_get_item_class($ditem->typ);
        if ($this->iscompleted) {
    ...
    protected function dependency_has_error($item) {
        ...
        if ($index1 >= $index2) {
            // Dependent item is after the current item in the patientform.
            return true;
        }
    ...
    /**
     * Returns a value stored for this item in the patientform (temporary or not, depending on the mode)
     * @param stdClass $item
     * @return string
     */
     ...
     *
     * @return array the responses (from the patientform_valuetmp table)
     * @since  Moodle 3.3
     */
    public function get_unfinished_responses() {
        ...
        if ($completedtmp) {
            $responses = $DB->get_records('patientform_valuetmp', ['completed' => $completedtmp->id]);
        }
        return $responses;
    }

    /**
     * Returns all temporary values for this patientform or just a value for an item
     * @param stdClass $item
     * @return array
     */
     ...
     *
     * @return array the responses (from the patientform_value table)
     * @since  Moodle 3.3
     */
    public function get_finished_responses() {
        ...
        if ($this->completed) {
            $responses = $DB->get_records('patientform_value', ['completed' => $this->completed->id]);
        }
        return $responses;
    }

    /**
     * Returns all completed values for this patientform or just a value for an item
     * @param stdClass $item
     * @return array
     */
    ...
    /**
     * Splits the patientform items into pages
     *
    ...
    /**
     * Get the next page for the patientform
     *
    ...
    public function get_next_page($gopage, $strictcheck = true) {
        ...
        // No further pages in the patientform have any visible items.
        return null;
    }

    /**
     * Get the previous page for the patientform
     *
    ...
    /**
     * Page index to resume the patientform
     *
     * When user abandones answering patientform and then comes back to it we should send him
    ...
    /**
     * Creates a new record in the 'patientform_completedtmp' table for the current user/guest session
     *
     * @return stdClass record from patientform_completedtmp or false if not found
     */
    protected function create_current_completed_tmp() {
        global $USER, $DB;
        $record = (object)['patientform' => $this->patientform->id];
        ...
        $record->anonymous_response = $this->patientform->anonymous;
        $id = $DB->insert_record('patientform_completedtmp', $record);
        $this->completedtmp = $DB->get_record('patientform_completedtmp', ['id' => $id]);
        $this->valuestmp = null;
        return $this->completedtmp;
    }

    /**
     * If user has already completed the patientform, create the temproray values from last completed attempt
     *
     * @return stdClass record from patientformcompletedtmp or false if not found
     */
    public function create_completed_tmp_from_last_completed() {
        if (!$this->get_current_completed_tmp()) {
            ...
            if ($lastcompleted) {
                $this->completedtmp = patientform_set_tmp_values($lastcompleted);
            }
        }
        return $this->completedtmp;
    }
     ...
     *
     * @param stdClass $data data from the form mod_patientform_complete_form
     */
    public function save_response_tmp($data) {
        ...
        if (!$completedtmp = $this->get_current_completed_tmp()) {
            ...
            $currentime = time();
            $DB->update_record('patientform_completedtmp',
                    ['id' => $completedtmp->id, 'timemodified' => $currentime]);
            $completedtmp->timemodified = $currentime;
        }

        // Find all existing values.
        $existingvalues = $DB->get_records_menu('patientform_valuetmp',
                ['completed' => $completedtmp->id], '', 'item, id');

        // Loop through all patientform items and save the ones that are present in $data.
        $allitems = $this->get_items();
        foreach ($allitems as $item) {
            ...
            // Convert the value to string that can be stored in 'patientform_valuetmp' or 'patientform_value'.
            $itemobj = patientform_get_item_class($item->typ);
            $newvalue['value'] = $itemobj->create_value($data->$keyname);

            // Update or insert the value in the 'patientform_valuetmp' table.
            if (array_key_exists($item->id, $existingvalues)) {
                $newvalue['id'] = $existingvalues[$item->id];
                $DB->update_record('patientform_valuetmp', $newvalue);
            } else {
                $DB->insert_record('patientform_valuetmp', $newvalue);
            }
        }
    ...
    public function save_response() {
        ...
        $patientformcompleted = $this->find_last_completed();
        $patientformcompletedtmp = $this->get_current_completed_tmp();

        if (patientform_check_is_switchrole()) {
        ...
        // Save values.
        $completedid = patientform_save_tmp_values($patientformcompletedtmp, $patientformcompleted);
        $this->completed = $DB->get_record('patientform_completed', array('id' => $completedid));

        // Send email.
        if ($this->patientform->anonymous == PATIENTFORM_ANONYMOUS_NO) {
            patientform_send_email($this->cm, $this->patientform, $this->cm->get_course(), $USER, $this->completed);
        } else {
            patientform_send_email_anonym($this->cm, $this->patientform, $this->cm->get_course());
        }

        unset($SESSION->patientform->is_started);

        // Update completion state.
        $completion = new completion_info($this->cm->get_course());
        if (isloggedin() && !isguestuser() && $completion->is_enabled($this->cm) &&
                $this->cm->completion == COMPLETION_TRACKING_AUTOMATIC && $this->patientform->completionsubmit) {
    ...
    protected function delete_completedtmp() {
        ...
        if ($completedtmp = $this->get_current_completed_tmp()) {
            $DB->delete_records('patientform_valuetmp', ['completed' => $completedtmp->id]);
            $DB->delete_records('patientform_completedtmp', ['id' => $completedtmp->id]);
     ...
     *
     * @return stdClass record from patientform_completed or false if not found
     */
    public function find_last_completed() {
        global $USER, $DB;
        if (!isloggedin() || isguestuser()) {
            // Not possible to retrieve completed patientform for guests.
            return false;
        }
        if ($this->is_anonymous()) {
            // Not possible to retrieve completed anonymous patientform.
            return false;
        }
        $params = array('patientform' => $this->patientform->id, 'userid' => $USER->id, 'anonymous_response' => PATIENTFORM_ANONYMOUS_NO);
        if ($this->get_courseid()) {
            $params['courseid'] = $this->get_courseid();
        }
        $this->completed = $DB->get_record('patientform_completed', $params);
        return $this->completed;
    }

    /**
     * Checks if current user has capability to submit the patientform
     *
     * There is an exception for fully anonymous patientforms when guests can complete
     * patientform without the proper capability.
     *
     * This should be followed by checking {@link can_submit()} because even if
     * user has capablity to complete, they may have already submitted patientform
    ...
    public function can_complete() {
        ...
        if (!empty($CFG->patientform_allowfullanonymous)
                    AND $this->patientform->course == SITEID
                    AND $this->patientform->anonymous == PATIENTFORM_ANONYMOUS_YES
                    AND (!isloggedin() OR isguestuser())) {
            // Guests are allowed to complete fully anonymous patientform without having 'mod/patientform:complete' capability.
            return true;
        }
    ...
    public function can_submit() {
        if ($this->get_patientform()->multiple_submit == 0 ) {
    ...
    public function trigger_module_viewed() {
        $event = \mod_patientform\event\course_module_viewed::create_from_record($this->patientform, $this->cm, $this->cm->get_course());
        $event->trigger();
    }
    ...
    /**
     * Process a page jump via the mod_patientform_complete_form.
     *
    ...
    public function process_page($gopage, $gopreviouspage = false) {
        ...
        $this->form = new mod_patientform_complete_form(mod_patientform_complete_form::MODE_COMPLETE,
            $this, 'patientform_complete_form', array('gopage' => $gopage));

        if ($this->form->is_cancelled()) {
            // Form was cancelled - return to the course page.
            $urltogo = course_get_url($this->courseid ?: $this->patientform->course);
            ...
            if (!isset($SESSION->patientform->is_started) OR !$SESSION->patientform->is_started == true) {
            ...
            if (!empty($data->savevalues) || !empty($data->gonextpage)) {
                if (($nextpage = $this->get_next_page($gopage)) !== null) {
                    ...
                    $this->save_response();
                    if (!$this->get_patientform()->page_after_submit) {
                        \core\notification::success(get_string('entries_saved', 'patientform'));
                    }
