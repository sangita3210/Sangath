...
/**
 * Class for exporting a patientform item (question).
 *
...
use mod_patientform\patientform;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class patientform_item_exporter extends exporter {
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $itemobj = patientform_get_item_class($this->data->typ);
        $values = array(
            'itemfiles' => array(),
        ...
        $fs = get_file_storage();
        $files = array();
        $itemfiles = $fs->get_area_files($context->id, 'mod_patientform', 'item', $this->data->id, 'filename', false);
        if (!empty($itemfiles)) {
