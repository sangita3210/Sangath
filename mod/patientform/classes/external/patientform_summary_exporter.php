...
/**
 * Class for exporting partial patientform data.
 *
 * @package    mod_patientform
 * @copyright  2017 Juan Leyva <juan@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
namespace mod_patientform\external;
...
/**
 * Class for exporting partial patientform data (some fields are only viewable by admins).
 *
 * @copyright  2017 Juan Leyva <juan@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class patientform_summary_exporter extends exporter {
    ...
    protected static function define_properties() {
        return array(
            ...
            'course' => array(
                'type' => PARAM_INT,
                'description' => 'Course id this patientform is part of.',
            ),
            'name' => array(
                'type' => PARAM_TEXT,
                'description' => 'patientform name.',
            ),
            'intro' => array(
                'default' => '',
                'type' => PARAM_RAW,
                'description' => 'patientform introduction text.',
            ),
            'introformat' => array(
                'choices' => array(FORMAT_HTML, FORMAT_MOODLE, FORMAT_PLAIN, FORMAT_MARKDOWN),
                'type' => PARAM_INT,
                'default' => FORMAT_MOODLE,
                'description' => 'patientform intro text format.',
            ),
            'anonymous' => array(
                'type' => PARAM_INT,
                'description' => 'Whether the patientform is anonymous.',
            ),
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $values['introfiles'] = external_util::get_area_files($context->id, 'mod_patientform', 'intro', false, false);

        if (!empty($this->data->page_after_submit)) {
            $values['pageaftersubmitfiles'] = external_util::get_area_files($context->id, 'mod_patientform', 'page_after_submit');
        }
