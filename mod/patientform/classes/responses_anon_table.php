...
/**
 * Contains class mod_patientform_responses_anon_table
 *
...
/**
 * Class mod_patientform_responses_anon_table
 *
 * @package   mod_patientform
 * @copyright 2016 Marina Glancy
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_patientform_responses_anon_table extends mod_patientform_responses_table {
    ...
    public function init($group = 0) {
        ...
        $cm = $this->patientformstructure->get_cm();
        $this->uniqueid = 'patientform-showentry-anon-list-' . $cm->instance;

        // There potentially can be both tables with anonymouns and non-anonymous responses on
        // the same page (for example when patientform anonymity was changed after some people
        ...
        $tableheaders = [get_string('response_nr', 'patientform')];
        ...
        $params = ['instance' => $cm->instance,
            'anon' => PATIENTFORM_ANONYMOUS_YES,
            'courseid' => $this->patientformstructure->get_courseid()];

        $fields = 'c.id, c.random_response, c.courseid';
        $from = '{patientform_completed} c';
        $where = 'c.anonymous_response = :anon AND c.patientform = :instance';
    ...
    public function col_random_response($row) {
        if ($this->is_downloading()) {
            ...
            return html_writer::link($this->get_link_single_entry($row),
                    get_string('response_nr', 'patientform').': '. $row->random_response);
        }
    }
