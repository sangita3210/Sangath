...
/**
 * Contains class mod_patientform_responses_table
 *
...
/**
 * Class mod_patientform_responses_table
 *
 * @package   mod_patientform
 * @copyright 2016 Marina Glancy
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_patientform_responses_table extends table_sql {

    /**
     * Maximum number of patientform questions to display in the "Show responses" table
     */
    const PREVIEWCOLUMNSLIMIT = 10;

    /**
     * Maximum number of patientform questions answers to retrieve in one SQL query.
     ...
     */
    public function __construct(mod_patientform_structure $patientformstructure, $group = 0) {
        $this->patientformstructure = $patientformstructure;

        parent::__construct('patientform-showentry-list-' . $patientformstructure->get_cm()->instance);

        $this->showall = optional_param($this->showallparamname, 0, PARAM_BOOL);
        $this->define_baseurl(new moodle_url('/mod/patientform/show_entries.php',
            ['id' => $this->patientformstructure->get_cm()->id]));
        if ($courseid = $this->patientformstructure->get_courseid()) {
        ...
        $name = format_string($patientformstructure->get_patientform()->name);
        $this->is_downloadable(true);
        $this->is_downloading(optional_param($this->downloadparamname, 0, PARAM_ALPHA),
                $name, get_string('responses', 'patientform'));
    ...
    protected function init($group = 0) {
        ...
        if ($this->patientformstructure->get_patientform()->course == SITEID && !$this->patientformstructure->get_courseid()) {
        ...
        $params['instance'] = $this->patientformstructure->get_patientform()->id;
        $params['notdeleted'] = 0;
        $params['courseid'] = $this->patientformstructure->get_courseid();

        $group = (empty($group)) ? groups_get_activity_group($this->patientformstructure->get_cm(), true) : $group;
        if ($group) {
    ...
    protected function get_context() {
        return context_module::instance($this->patientformstructure->get_cm()->id);
    }
    ...
    public function other_cols($column, $row) {
        if (preg_match('/^val(\d+)$/', $column, $matches)) {
            $items = $this->patientformstructure->get_items();
            $itemobj = patientform_get_item_class($items[$matches[1]]->typ);
    ...
    public function col_userpic($row) {
        ...
        return $OUTPUT->user_picture($user, array('courseid' => $this->patientformstructure->get_cm()->course));
    }
    ...
    public function col_deleteentry($row) {
        ...
        $deleteaction = new confirm_action(get_string('confirmdeleteentry', 'patientform'));
        return $OUTPUT->action_icon($deleteentryurl,
            new pix_icon('t/delete', get_string('delete_entry', 'patientform')), $deleteaction);
    }
    ...
    public function col_courseid($row) {
        $courses = $this->patientformstructure->get_completed_courses();
        $name = '';
    ...
    protected function add_all_values_to_output() {
        ...
        $items = $this->patientformstructure->get_items(true);
        if (!$this->is_downloading()) {
        ...
        // Add patientform response values.
        foreach ($items as $nr => $item) {
            if ($columnscount++ < self::TABLEJOINLIMIT) {
                ...
                $this->sql->from .= " LEFT OUTER JOIN {patientform_value} v{$nr} " .
        ...
        if (!$this->is_downloading() && has_capability('mod/patientform:deletesubmissions', $this->get_context())) {
    ...
    public function display() {
        global $OUTPUT;
        groups_print_activity_menu($this->patientformstructure->get_cm(), $this->baseurl->out());
        ...
        if (count($this->patientformstructure->get_items(true)) > self::PREVIEWCOLUMNSLIMIT) {
            echo $OUTPUT->notification(get_string('questionslimited', 'patientform', self::PREVIEWCOLUMNSLIMIT), 'info');
        }

        $this->out($this->showall ? $grandtotal : PATIENTFORM_DEFAULT_PAGE_COUNT,
                $grandtotal > PATIENTFORM_DEFAULT_PAGE_COUNT);

        // Toggle 'Show all' link.
        if ($this->totalrows > PATIENTFORM_DEFAULT_PAGE_COUNT) {
            if (!$this->use_pages) {
                echo html_writer::div(html_writer::link(new moodle_url($this->baseurl, [$this->showallparamname => 0]),
                        get_string('showperpage', '', PATIENTFORM_DEFAULT_PAGE_COUNT)), 'showall');
            } else {
    ...
    public function get_reponse_navigation_links($record) {
        ...
        while ($this->rawdata->valid()) {
            ...
            if ($row->id == $record->id) {
                $page = $this->showall ? 0 : floor($counter / PATIENTFORM_DEFAULT_PAGE_COUNT);
                $thisrow = $row;
                $this->rawdata->next();
    ...
    protected function build_table_chunk(&$rows, &$columnsgroups) {
        ...
        foreach ($columnsgroups as $columnsgroup) {
            ...
            foreach ($columnsgroup as $nr => $item) {
                ...
                $from .= " LEFT OUTER JOIN {patientform_value} v{$nr} " .
                    "ON v{$nr}.completed = c.id AND v{$nr}.item = :itemid{$nr}";
