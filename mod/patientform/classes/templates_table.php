...
/**
 * Contains class mod_patientform_templates_table
 *
...
/**
 * Class mod_patientform_templates_table
 *
 * @package   mod_patientform
 * @copyright 2016 Marina Glancy
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_patientform_templates_table extends flexible_table {
    ...
    public function __construct($uniqueid, $baseurl) {
        ...
        $tableheaders = array(get_string('template', 'patientform'), '');
    ...
    public function display($templates) {
        global $OUTPUT;
        if (empty($templates)) {
            echo $OUTPUT->box(get_string('no_templates_available_yet', 'patientform'),
        ...
        foreach ($templates as $template) {
            ...
            $deleteaction = new confirm_action(get_string('confirmdeletetemplate', 'patientform'));
            $data[] = $OUTPUT->action_icon($url, new pix_icon('t/delete', $strdeletepatientform), $deleteaction);
            $this->add_data($data);
        }
        $this->finish_output();
    }
