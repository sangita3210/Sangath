...
class patientform_edit_use_template_form extends moodleform {
    ...
    public function definition() {
        ...
        $elementgroup = array();
        //headline
        $mform->addElement('header', 'using_templates', get_string('using_templates', 'patientform'));
        // hidden elements
        ...
        $owntemplates = patientform_get_template_list($course, 'own');
        $publictemplates = patientform_get_template_list($course, 'public');
        ...
        if ($owntemplates or $publictemplates) {
            ...
            if ($publictemplates) {
                ...
                $options[get_string('public', 'patientform')] = $publicoptions;
            }
            ...
            $elementgroup[] = $mform->createElement('selectgroups',
                                                     'templateid',
                                                     get_string('using_templates', 'patientform'),
                                                     $options,
                                                     $attributes);

            $elementgroup[] = $mform->createElement('submit',
                                                     'use_template',
                                                     get_string('use_this_template', 'patientform'),
                                                     array('class' => 'hiddenifjs'));

            $mform->addGroup($elementgroup, 'elementgroup', '', array(' '), false);
        } else {
            $mform->addElement('static', 'info', get_string('no_templates_available_yet', 'patientform'));
        }
...
class patientform_edit_create_template_form extends moodleform {
    ...
    public function definition() {
        ...
        //headline
        $mform->addElement('header', 'creating_templates', get_string('creating_templates', 'patientform'));
        ...
        $elementgroup[] = $mform->createElement('text',
                                                 'templatename',
                                                 get_string('name', 'patientform'),
                                                 array('size'=>'40', 'maxlength'=>'200'));

        if (has_capability('mod/patientform:createpublictemplate', context_system::instance())) {
            $elementgroup[] = $mform->createElement('checkbox',
                                                     'ispublic', '',
                                                     get_string('public', 'patientform'));
        }
        ...
        // Buttons.
        $mform->addElement('submit', 'create_template', get_string('save_as_new_template', 'patientform'));
    ...
    public function validation($data, $files) {
        ...
        if (!isset($data['templatename']) || trim(strval($data['templatename'])) === '') {
            $errors['elementgroup'] = get_string('name_required', 'patientform');
        }
        return $errors;
    }
}
