...
$url = new moodle_url('/mod/patientform/export.php', array('id'=>$id));
if ($action !== false) {
...
if ($action == 'exportfile') {
    if (!$exportdata = patientform_get_xml_data($patientform->id)) {
        print_error('nodata');
    }
    @patientform_send_xml_data($exportdata, 'patientform_'.$patientform->id.'.xml');
    exit;
}
...
function patientform_get_xml_data($patientformid) {
    global $DB;

    $space = '     ';
    //get all items of the patientform
    if (!$items = $DB->get_records('patientform_item', array('patientform'=>$patientformid), 'position')) {
        return false;
    }

    //writing the header of the xml file including the charset of the currrent used language
    $data = '<?xml version="1.0" encoding="UTF-8" ?>'."\n";
    $data .= '<PATIENTFORM VERSION="200701" COMMENT="XML-Importfile for mod/patientform">'."\n";
    $data .= $space.'<ITEMS>'."\n";
...
function patientform_send_xml_data($data, $filename) {
