...
$url = new moodle_url('/mod/patientform/import.php', array('id'=>$id));
if ($choosefile !== false) {
...
if (! $cm = get_coursemodule_from_id('patientform', $id)) {
...
$mform = new patientform_import_form();
...
if ($choosefile) {
    ...
    if (!$xmldata = patientform_load_xml_data($xmlcontent)) {
        print_error('cannotloadxml', 'patientform', 'edit.php?id='.$id);
    }

    $importerror = patientform_import_loaded_data($xmldata, $patientform->id);
    if ($importerror->stat == true) {
        $url = 'edit.php?id='.$id.'&do_show=templates';
        redirect($url, get_string('import_successfully', 'patientform'), 3);
        exit;
    }
}
...
///////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
echo $OUTPUT->heading(get_string('import_questions', 'patientform'), 3);
...
function patientform_load_xml_data($xmlcontent) {
    global $CFG;
    require_once($CFG->dirroot.'/lib/xmlize.php');

    if (!$xmlcontent = patientform_check_xml_utf8($xmlcontent)) {
        return false;
    }

    $data = xmlize($xmlcontent, 1, 'UTF-8');

    if (intval($data['PATIENTFORM']['@']['VERSION']) != 200701) {
        return false;
    }
    $data = $data['PATIENTFORM']['#']['ITEMS'][0]['#']['ITEM'];
    return $data;
}

function patientform_import_loaded_data(&$data, $patientformid) {
    global $CFG, $DB;

    patientform_load_patientform_items();
    ...
    if (!is_array($data)) {
        $error->msg[] = get_string('data_is_not_an_array', 'patientform');
        $error->stat = false;
        return $error;
    }

    if ($deleteolditems) {
        patientformk_delete_all_items($patientformid);
        $position = 0;
    } else {
        //items will be add to the end of the existing items
        $position = $DB->count_records('patientform_item', array('patientform'=>$patientformid));
    }
    ...
    foreach ($data as $item) {
        ...
        $newitem->patientform = $patientformid;
        $newitem->template = 0;
        $newitem->typ = $typ;
        ...
        $newid = $DB->insert_record('patientform_item', $newitem);
...
function patientform_check_xml_utf8($text) {
    //find the encoding
