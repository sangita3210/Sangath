...
class patientform_item_info extends patientform_item_base {
    protected $type = "info";

    /** Mode recording response time (for non-anonymous patients only) */
    ...
    public function build_editform($item, $patientform, $cm) {
        ...
        if ($patientform->anonymous == PATIENTFORM_ANONYMOUS_NO || $item->presentation == self::MODE_RESPONSETIME) {
            // "Response time" is hidden anyway in case of anonymous patientform, no reason to offer this option.
            // However if it was already selected leave it in the dropdown.
            $presentationoptions[self::MODE_RESPONSETIME] = get_string('responsetime', 'patientform');
        }
        $presentationoptions[self::MODE_COURSE]  = get_string('course');
        $presentationoptions[self::MODE_CATEGORY]  = get_string('coursecategory');

        //build the form
        $this->item_form = new patientform_info_form('edit_item.php',
                                                  array('item'=>$item,
                                                  'common'=>$commonparams,
    ...
    protected function get_analysed($item, $groupid = false, $courseid = false) {
        ...
        $values = patientform_get_group_values($item, $groupid, $courseid);
        if ($values) {
            $data = array();
     ...
     * @return string
     */
    protected function get_current_value($item, $patientform, $courseid) {
        global $DB;
        switch ($item->presentation) {
            case self::MODE_RESPONSETIME:
                if ($patientform->anonymous != PATIENTFORM_ANONYMOUS_YES) {
                    // Response time is not allowed in anonymous patientforms.
                    return time();
                }
                break;
            case self::MODE_COURSE:
    ...
    public function complete_form_element($item, $form) {
        if ($form->get_mode() == mod_patientform_complete_form::MODE_VIEW_RESPONSE) {
            $value = strval($form->get_item_value($item));
        } else {
            $value = $this->get_current_value($item,
                    $form->get_patientform(), $form->get_current_course_id());
        }
    ...
    public function get_data_for_external($item) {
        global $DB;
        $patientform = $DB->get_record('patientform', array('id' => $item->patientform), '*', MUST_EXIST);
        // Return the default value (course name, category name or timestamp).
        return $this->get_current_value($item, $patientform, $patientform->course);
    }
