...
class patientform_item_label extends patientform_item_base {
    ...
    public function build_editform($item, $patientform, $cm) {
        ...
        $this->item_form = new patientform_label_form('edit_item.php', $customdata);
    }
    ...
    private function print_item($item) {
        ...
        if (!$item->patientform AND $item->template) {
            $template = $DB->get_record('patientform_template', array('id'=>$item->template));
            ...
            $cm = get_coursemodule_from_instance('patientform', $item->patientform);
