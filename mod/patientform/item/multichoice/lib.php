...
define('PATIENTFORM_MULTICHOICE_TYPE_SEP', '>>>>>');
define('PATIENTFORM_MULTICHOICE_LINE_SEP', '|');
define('PATIENTFORM_MULTICHOICE_ADJUST_SEP', '<<<<<');
define('PATIENTFORM_MULTICHOICE_IGNOREEMPTY', 'i');
define('PATIENTFORM_MULTICHOICE_HIDENOSELECT', 'h');

class patientform_item_multichoice extends patientform_item_base {
    ...
    public function build_editform($item, $patientform, $cm) {
        ...
        $this->item_form = new patientform_multichoice_form('edit_item.php', $customdata);
    }
    ...
    protected function get_analysed($item, $groupid = false, $courseid = false) {
        ...
        $answers = null;
        $answers = explode (PATIENTFORM_MULTICHOICE_LINE_SEP, $info->presentation);
        if (!is_array($answers)) {
            return null;
        }
        ...
        if ($info->subtype == 'c') {
            ...
            for ($i = 1; $i <= $sizeofanswers; $i++) {
                ...
                foreach ($values as $value) {
                    ...
                    $vallist = explode(PATIENTFORM_MULTICHOICE_LINE_SEP, $value->value);
    ...
    public function get_printval($item, $value) {
        ...
        $presentation = explode (PATIENTFORM_MULTICHOICE_LINE_SEP, $info->presentation);

        if ($info->subtype == 'c') {
            $vallist = array_values(explode (PATIENTFORM_MULTICHOICE_LINE_SEP, $value->value));
    ...
    public function complete_form_element($item, $form) {
        ...
        if ($info->subtype === 'd' || ($info->subtype === 'r' && $form->is_frozen())) {
            ...
            $objs = [];
            foreach (explode(PATIENTFORM_MULTICHOICE_LINE_SEP, $form->get_item_value($item)) as $v) {
            ...
            if ($info->subtype === 'c') {
                ...
                $element = $form->add_form_group_element($item, 'group_'.$inputname, $name, $objs, $separator, $class);
                if ($tmpvalue) {
                    foreach (explode(PATIENTFORM_MULTICHOICE_LINE_SEP, $tmpvalue) as $v) {
    ...
    public function create_value($value) {
        ...
        return join(PATIENTFORM_MULTICHOICE_LINE_SEP, $value);
    }
    ...
    public function compare_value($item, $dbvalue, $dependvalue) {
        ...
        if (is_array($dbvalue)) {
            ...
            $dbvalues = explode(PATIENTFORM_MULTICHOICE_LINE_SEP, $dbvalue);
        }
    ...
    public function get_info($item) {
        ...
        $parts = explode(PATIENTFORM_MULTICHOICE_TYPE_SEP, $item->presentation);
        ...
        if ($info->subtype != 'd') {
            $parts = explode(PATIENTFORM_MULTICHOICE_ADJUST_SEP, $info->presentation);
    ...
    public function set_ignoreempty($item, $ignoreempty=true) {
        $item->options = str_replace(PATIENTFORM_MULTICHOICE_IGNOREEMPTY, '', $item->options);
        if ($ignoreempty) {
            $item->options .= PATIENTFORM_MULTICHOICE_IGNOREEMPTY;
        }
    }

    public function ignoreempty($item) {
        if (strstr($item->options, PATIENTFORM_MULTICHOICE_IGNOREEMPTY)) {
            return true;
        }
        return false;
    }
    ...
    public function set_hidenoselect($item, $hidenoselect=true) {
        $item->options = str_replace(PATIENTFORM_MULTICHOICE_HIDENOSELECT, '', $item->options);
        if ($hidenoselect) {
            $item->options .= PATIENTFORM_MULTICHOICE_HIDENOSELECT;
        }
    }

    public function hidenoselect($item) {
        if (strstr($item->options,PATIENTFORM_MULTICHOICE_HIDENOSELECT)) {
            return true;
        }
        return false;
    }
