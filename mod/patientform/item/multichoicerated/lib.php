...
define('PATIENTFORM_RADIORATED_ADJUST_SEP', '<<<<<');

define('PATIENTFORM_MULTICHOICERATED_MAXCOUNT', 10); //count of possible items
define('PATIENTFORM_MULTICHOICERATED_VALUE_SEP', '####');
define('PATIENTFORM_MULTICHOICERATED_VALUE_SEP2', '/');
define('PATIENTFORM_MULTICHOICERATED_TYPE_SEP', '>>>>>');
define('PATIENTFORM_MULTICHOICERATED_LINE_SEP', '|');
define('PATIENTFORM_MULTICHOICERATED_ADJUST_SEP', '<<<<<');
define('PATIENTFORM_MULTICHOICERATED_IGNOREEMPTY', 'i');
define('PATIENTFORM_MULTICHOICERATED_HIDENOSELECT', 'h');

class patientform_item_multichoicerated extends patientform_item_base {
    ...
    public function build_editform($item, $patientform, $cm) {
        ...
        //get the lastposition number of the patientform_items
        ...
        $patientformitems = patientform_get_depend_candidates_for_item($patientform, $item);
        $commonparams = array('cmid'=>$cm->id,
                             'id'=>isset($item->id) ? $item->id : null,
                             'typ'=>$item->typ,
                             'items'=>$patientformitems,
        ...
        $this->item_form = new patientform_multichoicerated_form('edit_item.php', $customdata);
    }
    ...
    public function save_item() {
        ...
        if (!$item->id) {
            ...
            $DB->update_record('patientform_item', $item);
        }

        return $DB->get_record('patientform_item', array('id'=>$item->id));
    }
     ...
     *
     * @param stdClass $item the db-object from patientform_item
     * @param int $groupid
     * @param int $courseid
     * @return array
     */
    protected function get_analysed($item, $groupid = false, $courseid = false) {
        ...
        $info = $this->get_info($item);
        $lines = null;
        $lines = explode (PATIENTFORM_MULTICHOICERATED_LINE_SEP, $info->presentation);
        if (!is_array($lines)) {
            return null;
        }

        //die Werte holen
        $values = patientform_get_group_values($item, $groupid, $courseid, $this->ignoreempty($item));
        if (!$values) {
            return null;
        }
        ...
        for ($i = 1; $i <= $sizeoflines; $i++) {
            $item_values = explode(PATIENTFORM_MULTICHOICERATED_VALUE_SEP, $lines[$i-1]);
            $ans = new stdClass();
    ...
    public function get_printval($item, $value) {
        ...
        $presentation = explode (PATIENTFORM_MULTICHOICERATED_LINE_SEP, $info->presentation);
        $index = 1;
        foreach ($presentation as $pres) {
            if ($value->value == $index) {
                $item_label = explode(PATIENTFORM_MULTICHOICERATED_VALUE_SEP, $pres);
    ...
    public function print_analysed($item, $itemnr = '', $groupid = false, $courseid = false) {
        ...
        if ($analysed_item) {
            ...
            $series = new \core\chart_series(format_string(get_string("responses", "patientform")), $data['series']);
            ...
            echo get_string('average', 'patientform').': '.$avg.'</b>';
            echo '</td></tr>';
        }
    }
    ...
    public function excelprint_item(&$worksheet, $row_offset,
        ...
        if (is_array($data)) {
            ...
            $worksheet->write_string($row_offset,
                                count($data) + 2,
                                get_string('average', 'patientform'),
                                $xls_formats->value_bold);
    ...
    protected function get_options($item) {
        ...
        foreach ($lines as $idx => $line) {
            list($weight, $optiontext) = explode(PATIENTFORM_MULTICHOICERATED_VALUE_SEP, $line);
        ...
        if ($info->subtype === 'r' && !$this->hidenoselect($item)) {
            $options = array(0 => get_string('not_selected', 'patientform')) + $options;
        }
     ...
     *
     * @param stdClass $item
     * @param mod_patientform_complete_form $form
     */
    public function complete_form_element($item, $form) {
        ...
        if ($info->subtype === 'd' || $form->is_frozen()) {
            ...
            $objs[] = ['static', '', '', html_writer::span('', '', ['id' => 'patientform_item_' . $item->id])];
    ...
    public function compare_value($item, $dbvalue, $dependvalue) {
        ...
        if (is_array($dbvalue)) {
            ...
            $dbvalues = explode(PATIENTFORM_MULTICHOICERATED_LINE_SEP, $dbvalue);
        }
        ...
        $index = 1;
        foreach ($presentation as $pres) {
            $presvalues = explode(PATIENTFORM_MULTICHOICERATED_VALUE_SEP, $pres);
    ...
    public function get_info($item) {
        ...
        $parts = explode(PATIENTFORM_MULTICHOICERATED_TYPE_SEP, $item->presentation);
        ...
        if ($info->subtype != 'd') {
            $parts = explode(PATIENTFORM_MULTICHOICERATED_ADJUST_SEP, $info->presentation);
        ...
        $info->values = $this->prepare_presentation_values_print($info->presentation,
                                                    PATIENTFORM_MULTICHOICERATED_VALUE_SEP,
                                                    PATIENTFORM_MULTICHOICERATED_VALUE_SEP2);
        return $info;
    }
    ...
    public function prepare_presentation_values_print($valuestring, $valuesep1, $valuesep2) {
        ...
        return $this->prepare_presentation_values(PATIENTFORM_MULTICHOICERATED_LINE_SEP,
                                                  "\n",
                                                  $valuestring,
                                                  $valuesep1,
                                                  $valuesep2);
    ...
    public function prepare_presentation_values_save($valuestring, $valuesep1, $valuesep2) {
        ...
        return $this->prepare_presentation_values("\n",
                       PATIENTFORM_MULTICHOICERATED_LINE_SEP,
                        $valuestring,
                        $valuesep1,
                        $valuesep2);
    }
    ...
    public function set_ignoreempty($item, $ignoreempty=true) {
        $item->options = str_replace(PATIENTFORM_MULTICHOICERATED_IGNOREEMPTY, '', $item->options);
        if ($ignoreempty) {
            $item->options .= PATIENTFORM_MULTICHOICERATED_IGNOREEMPTY;
        }
    }

    public function ignoreempty($item) {
        if (strstr($item->options, PATIENTFORM_MULTICHOICERATED_IGNOREEMPTY)) {
            return true;
        }
        return false;
    }
    ...
    public function set_hidenoselect($item, $hidenoselect=true) {
        $item->options = str_replace(PATIENTFORM_MULTICHOICERATED_HIDENOSELECT, '', $item->options);
        if ($hidenoselect) {
            $item->options .= PATIENTFORM_MULTICHOICERATED_HIDENOSELECT;
        }
    }

    public function hidenoselect($item) {
        if (strstr($item->options, PATIENTFORM_MULTICHOICERATED_HIDENOSELECT)) {
            return true;
        }
        return false;
    }
