...
abstract class patientform_item_base {
     ...
     */
    abstract public function build_editform($item, $patientform, $cm);
     ...
     * @param mixed $value element from mod_patientform_complete_form::get_data() with the name $item->typ.'_'.$item->id
     * @return string
     */
     ...
     * @param object $item the db-object from patientform_item
     ...
     *
     * @param $item the db-object from patientform_item
     * @param string $itemnr
     * @param integer $groupid
     * @param integer $courseid
     ...
     *
     * @param object $item the db-object from patientform_item
     * @param string $value a item-related value from patientform_values
     * @return string
     */
     ...
     * - to display the form when user completes patientform
     * - to display existing elements when teacher edits the patientform items
     * - to display the patientform preview (print.php)
     * - to display the completed response
     * - to preview a patientform template
     *
    ...
    public function edit_actions($item, $patientform, $cm) {
        ...
        $strupdate = get_string('edit_item', 'patientform');
        $actions['update'] = new action_menu_link_secondary(
            new moodle_url('/mod/patientform/edit_item.php', array('id' => $item->id)),
        ...
        if ($this->can_switch_require()) {
            if ($item->required == 1) {
                $buttontitle = get_string('switch_item_to_not_required', 'patientform');
                $buttonimg = 'required';
            } else {
                $buttontitle = get_string('switch_item_to_required', 'patientform');
                $buttonimg = 'notrequired';
            }
            $actions['required'] = new action_menu_link_secondary(
                new moodle_url('/mod/patientform/edit.php', array('id' => $cm->id,
                    'switchitemrequired' => $item->id, 'sesskey' => sesskey())),
                new pix_icon($buttonimg, $buttontitle, 'patientform', array('class' => 'iconsmall', 'title' => '')),
                $buttontitle,
                array('class' => 'editing_togglerequired', 'data-action' => 'togglerequired')
            );
        }

        $strdelete = get_string('delete_item', 'patientform');
        $actions['delete'] = new action_menu_link_secondary(
            new moodle_url('/mod/patientform/edit.php', array('id' => $cm->id, 'deleteitem' => $item->id, 'sesskey' => sesskey())),
...
class patientform_item_pagebreak extends patientform_item_base {
    ...
    public function complete_form_element($item, $form) {
        $form->add_form_element($item,
            ['static',
                $item->typ.'_'.$item->id,
                '',
                html_writer::empty_tag('hr', ['class' => 'patientform_pagebreak', 'id' => 'patientform_item_' . $item->id])
            ]);
    }
    ...
    public function edit_actions($item, $patientform, $cm) {
        $actions = array();
        $strdelete = get_string('delete_pagebreak', 'patientform');
        $actions['delete'] = new action_menu_link_secondary(
            new moodle_url('/mod/patientform/edit.php', array('id' => $cm->id, 'deleteitem' => $item->id, 'sesskey' => sesskey())),
            new pix_icon('t/delete', $strdelete, 'moodle', array('class' => 'iconsmall', 'title' => '')),
            $strdelete,
