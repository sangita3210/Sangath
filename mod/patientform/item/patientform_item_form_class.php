...
define('PATIENTFORM_ITEM_NAME_TEXTBOX_SIZE', 80);
define('PATIENTFORM_ITEM_LABEL_TEXTBOX_SIZE', 20);
abstract class patientform_item_form extends moodleform {
    public function definition() {
        ...
        if (array_filter(array_keys($common['items']))) {
            $mform->addElement('select',
                                'dependitem',
                                get_string('dependitem', 'patientform').'&nbsp;',
                                $common['items']
                                );
            $mform->addHelpButton('dependitem', 'depending', 'patientform');
            $mform->addElement('text',
                                'dependvalue',
                                get_string('dependvalue', 'patientform'),
        ...
        $position_select = $mform->addElement('select',
                                            'position',
                                            get_string('position', 'patientform').'&nbsp;',
                                            $positionlist);
        ...
        $mform->addElement('hidden', 'patientform', $common['patientform']);
        $mform->setType('patientform', PARAM_INT);
        ...
        if (!empty($item->id)) {
            $buttonarray[] = &$mform->createElement('submit',
                                                    'update_item',
                                                    get_string('update_item', 'patientform'));

            $buttonarray[] = &$mform->createElement('submit',
                                                    'clone_item',
                                                    get_string('save_as_new_item', 'patientform'));
        } else {
            $mform->addElement('hidden', 'clone_item', 0);
            $mform->setType('clone_item', PARAM_INT);
            $buttonarray[] = &$mform->createElement('submit',
                                                    'save_item',
                                                    get_string('save_item', 'patientform'));
        }
