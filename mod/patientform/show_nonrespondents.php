...
$perpage = optional_param('perpage', PATIENTFORM_DEFAULT_PAGE_COUNT, PARAM_INT);  // how many per page
$showall = optional_param('showall', false, PARAM_INT);  // should we show all users
// $SESSION->patientform->current_tab = $do_show;
...
//this page only can be shown on nonanonymous patientforms in courses
//we should never reach this page
if ($patientform->anonymous != PATIENTFORM_ANONYMOUS_NO OR $patientform->course == SITEID) {
    print_error('error');
}

$url = new moodle_url('/mod/patientform/show_nonrespondents.php', array('id'=>$cm->id));
...
if ($action == 'sendmessage' AND has_capability('moodle/course:bulkmessaging', $coursecontext)) {
    ...
    $link2 = $CFG->wwwroot.'/mod/patientform/index.php?id='.$course->id;
    $link3 = $CFG->wwwroot.'/mod/patientform/view.php?id='.$cm->id;

    $htmlmessage .= '<div class="navbar">'.
    '<a target="_blank" href="'.$link1.'">'.$shortname.'</a> &raquo; '.
    '<a target="_blank" href="'.$link2.'">'.$strpatientforms.'</a> &raquo; '.
    '<a target="_blank" href="'.$link3.'">'.format_string($patientform->name, true).'</a>'.
    '</div>';
    ...
    if (is_array($messageuser)) {
        foreach ($messageuser as $userid) {
            ...
            $eventdata->contexturlname   = $patientform->name;
...
$baseurl = new moodle_url('/mod/patientform/show_nonrespondents.php');
...
$table = new flexible_table('patientform-shownonrespondents-'.$course->id);
...
$matchcount = patientform_count_incomplete_users($cm, $usedgroupid);
...
// Return students record including if they started or not the patientform.
$students = patientform_get_incomplete_users($cm, $usedgroupid, $sort, $startpage, $pagecount, true);
//####### viewreports-start
//print the list of students
echo $OUTPUT->heading(get_string('non_respondents_students', 'patientform', $matchcount), 4);
...
if (empty($students)) {
    ...
    if ($canbulkmessaging) {
        echo '<form class="mform" action="show_nonrespondents.php" method="post" id="patientform_sendmessageform">';
    }
    ...
    foreach ($students as $student) {
        ...
        if ($student->patientformstarted) {
            $data[] = get_string('started', 'patientform');
        } else {
            $data[] = get_string('not_started', 'patientform');
        }
    ...
    if ($showall) {
        ...
        echo $OUTPUT->container(html_writer::link($allurl, get_string('showperpage', '', PATIENTFORM_DEFAULT_PAGE_COUNT)),
                                    array(), 'showall');
    ...
    if (has_capability('moodle/course:bulkmessaging', $coursecontext)) {
        ...
        echo '<legend class="ftoggler">'.get_string('send_message', 'patientform').'</legend>';
        echo '<div>';
        echo '<label for="patientform_subject">'.get_string('subject', 'patientform').'&nbsp;</label>';
        echo '<input type="text" id="patientform_subject" size="50" maxlength="255" name="subject" value="'.s($subject).'" />';
        echo '</div>';
        print_textarea(true, 15, 25, 30, 10, "message", $message);
        print_string('formathtml');
        echo '<input type="hidden" name="format" value="'.FORMAT_HTML.'" />';
        echo '<br /><div class="buttons">';
        echo '<input type="submit" name="send_message" value="'.get_string('send', 'patientform').'" />';
        echo '</div>';
        ...
        $module = array('name'=>'mod_patientform', 'fullpath'=>'/mod/patientform/patientform.js');
        $PAGE->requires->js_init_call('M.mod_patientform.init_sendmessage', null, false, $module);
    }
}
