...
/**
 * Steps definitions related to mod_patientform
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_patientform extends behat_base {

    /**
     * Adds a question to the existing patientform with filling the form.
     *
     * The form for creating a question should be on one page.
     *
     * @When /^I add a "(?P<question_type_string>(?:[^"]|\\")*)" question to the patientform with:$/
     * @param string $questiontype
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_add_question_to_the_patientform_with($questiontype, TableNode $questiondata) {

        $questiontype = $this->escape($questiontype);
        $additem = $this->escape(get_string('add_item', 'patientform'));
        ...
        $saveitem = $this->escape(get_string('save_item', 'patientform'));
     ...
     * The form for creating a question should be on one page.
     *
     * @When /^I add a page break to the patientform$/
     */
    public function i_add_a_page_break_to_the_patientform() {

        $questiontype = $this->escape(get_string('add_pagebreak', 'patientform'));
        $additem = $this->escape(get_string('add_item', 'patientform'));
    ...
    /**
     * Quick way to generate answers to a one-page patientform.
     *
     * @When /^I log in as "(?P<user_name_string>(?:[^"]|\\")*)" and complete patientform "(?P<patientform_name_string>(?:[^"]|\\")*)" in course "(?P<course_name_string>(?:[^"]|\\")*)" with:$/
     * @param string $questiontype
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_log_in_as_and_complete_patientform_in_course($username, $patientformname, $coursename, TableNode $answers) {
        $username = $this->escape($username);
        $coursename = $this->escape($coursename);
        $patientformname = $this->escape($patientformname);
        $completeform = $this->escape(get_string('complete_the_form', 'patientform'));

        // Log in as user.
        $this->execute('behat_auth::i_log_in_as', $username);

        // Navigate to patientform complete form.
        $this->execute('behat_navigation::i_am_on_course_homepage', $coursename);
        $this->execute('behat_general::click_link', $patientformname);
    ...
    /**
     * Exports patientform and makes sure the export file is the same as in the fixture
     *
     * @Then /^following "(?P<link_string>(?:[^"]|\\")*)" should export patientform identical to "(?P<filename_string>(?:[^"]|\\")*)"$/
     * @param string $link
     * @param string $filename
     */
    public function following_should_export_patientform_identical_to($link, $filename) {
        global $CFG;
     ...
     *
     * @Then /^I show chart data for the "(?P<patientform_name_string>(?:[^"]|\\")*)" patientform$/
     * @param string $patientformname name of the patientform for which chart data needs to be shown.
     */
    public function i_show_chart_data_for_the_patientform($patientformname) {

        $patientformxpath = "//th[contains(normalize-space(string(.)), \"" . $patientformname . "\")]/ancestor::table/" .
        ...
        $charttabledataxpath = $patientformxpath .
    ...
    /**
     * Ensures two patientform export files are identical
     *
    ...
    protected function compare_exports($expected, $actual) {
        ...
        $dataexpected = $dataexpected['PATIENTFORM']['#']['ITEMS'][0]['#']['ITEM'];
        $dataactual = xmlize($actual, 1, 'UTF-8');
        $dataactual = $dataactual['PATIENTFORM']['#']['ITEMS'][0]['#']['ITEM'];
