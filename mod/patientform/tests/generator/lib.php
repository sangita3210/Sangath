...
/**
 * mod_patientform data generator.
 *
...
/**
 * mod_patientform data generator class.
 *
 * @package    mod_patientform
 * @category   test
 * @copyright  2013 Ankit Agarwal
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_patientform_generator extends testing_module_generator {
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        if (!isset($record->anonymous)) {
            $record->anonymous = PATIENTFORM_ANONYMOUS_YES;
        }
        ...
        // Hack to bypass draft processing of patientform_add_instance.
     ...
     *
     * @param object $patientform patientform record
     * @param array $record (optional) to override default values
     * @return int
     */
    public function create_item_info($patientform, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/patientform/lib.php');

        $itemobj = patientform_get_item_class('info');
        $position = $DB->count_records('patientform_item', array('patientform' => $patientform->id)) + 1;

        $record = (array)$record + array(
            'id' => 0,
            'patientform' => $patientform->id,
            'template' => 0,
            'name' => 'patientform question item ' . $position,
            'label' => 'patientform label ' . $position,
     ...
     * @return int
     */
    public function create_item_label($patientform, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/patientform/lib.php');

        $itemobj = patientform_get_item_class('label');
        $position = $DB->count_records('patientform_item', array('patientform' => $patientform->id)) + 1;
     ...
     * @return int
     */
    public function create_item_multichoice($patientform, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/patientform/lib.php');

        $itemobj = patientform_get_item_class('multichoice');
        $position = $DB->count_records('patientform_item', array('patientform' => $patientform->id)) + 1;
        ...
        $presentation = str_replace("\n", PATIENTFORM_MULTICHOICE_LINE_SEP, trim($record['values']));

        if ($record['horizontal'] == 1 AND $record['subtype'] != 'd') {
            $presentation .= PATIENTFORM_MULTICHOICE_ADJUST_SEP.'1';
        }
        $record['presentation'] = $record['subtype'].PATIENTFORM_MULTICHOICE_TYPE_SEP.$presentation;
     ...
     * @return int
     */
    public function create_item_multichoicerated($patientform, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/patientform/lib.php');

        $itemobj = patientform_get_item_class('multichoicerated');
        $position = $DB->count_records('patientform_item', array('patientform' => $patientform->id)) + 1;
        ...
        $itemobj = new patientform_item_multichoicerated();
        $presentation = $itemobj->prepare_presentation_values_save(trim($record['values']),
            PATIENTFORM_MULTICHOICERATED_VALUE_SEP2, PATIENTFORM_MULTICHOICERATED_VALUE_SEP);

        if ($record['horizontal'] == 1 AND $record['subtype'] != 'd') {
            $presentation .= PATIENTFORM_MULTICHOICERATED_ADJUST_SEP.'1';
        }
        $record['presentation'] = $record['subtype'].PATIENTFORM_MULTICHOICERATED_TYPE_SEP.$presentation;
     ...
     * @return int
     */
    public function create_item_numeric($patientform, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/patientform/lib.php');

        $itemobj = patientform_get_item_class('numeric');
        $position = $DB->count_records('patientform_item', array('patientform' => $patientform->id)) + 1;
     ...
     * @return int
     */
    public function create_item_textarea($patientform, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/patientform/lib.php');

        $itemobj = patient_get_item_class('textarea');
        $position = $DB->count_records('patientform_item', array('patientform' => $patientform->id)) + 1;
     ...
     * @return int
     */
    public function create_item_textfield($patientform, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/patientform/lib.php');

        $itemobj = patientform_get_item_class('textfield');
        $position = $DB->count_records('patientform_item', array('patientform' => $patientform->id)) + 1;
     ...
     */
    public function create_item_pagebreak($patientform) {
        global $CFG;
        require_once($CFG->dirroot.'/mod/patientform/lib.php');

        return patientform_create_pagebreak($patientform->id);
    }
}
