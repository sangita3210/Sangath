YUI.add('moodle-mod_patientform-dragdrop', function(Y) {
    var DRAGDROPNAME = 'mod_patientform_dragdrop';
    var CSS = {
        DRAGAREA: '#patientform_dragarea',
        DRAGITEMCLASS: 'patientform_itemlist',
        DRAGITEM: 'div.patientform_itemlist',
        DRAGLIST: '#patientform_dragarea form',
    ...
    Y.extend(DRAGDROP, M.core.dragdrop, {
        ...
        initializer : function(params) {
            ...
            var groups = ['patientformitem'];

            var handletitle = M.util.get_string('move_item', 'patientform');
            ...
            listitems = basenode.all(CSS.DRAGITEM).each(function(v) {
                var item_id = this.get_node_id(v.get('id')); //Get the id of the patientform item.
        ...
        drag_drophit_handler : function(e) {
            ...
            if (!drop.hasClass(CSS.DRAGITEMCLASS)) {
                ...
                drop.all(CSS.DRAGITEM).each(function(v) {
                    childElement = v.one('.felement').one('[id^="patientform_item_"]');
                    if (childElement) {
        ...
        save_item_order : function(cmid, itemorder, spinner) {
            ...
            Y.io(M.cfg.wwwroot + '/mod/patientform/ajax.php', {
         ...
         *
         * @param id The dom id, f.g.: patientform_item_22
         * @return int
         */
        get_node_id : function(id) {
            return Number(id.replace(/^.*patientform_item_/i, ''));
        }
    ...
    M.mod_patientform = M.mod_patientform || {};
    M.mod_patientform.init_dragdrop = function(params) {
